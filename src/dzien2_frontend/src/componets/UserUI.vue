<template>
    <div class="p-4">
      <div class="flex space-x-2 mb-4">
        <button class="bg-blue-600 text-white rounded px-4 py-2 hover:bg-blue-700" @click="show_report">Wyświetl awarie</button>git diff
        ...
        <div v-else class="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto">
          ...
          <div v-for="(komentarz, index) in report.komentarze" :key="index" class="bg-stone-300 p-4 rounded mt-4 drop-shadow-xl">
            <p class="flex items-center">Komentarz:&nbsp;&nbsp;
              <span v-if="editingComment !== index">  {{ komentarz }}  </span>
            ...
            <div class="flex space-x-2 mt-2" v-if="editingComment === index">
              <button class="bg-blue-600 text-white rounded px-4 py-2 hover:bg-blue-700" @click="saveComment(index)">Zapisz</button>
              <button class="bg-gray-600 text-white rounded px-4 py-2 hover:bg-gray-700" @click="cancelEditComment">Anuluj</button>
              <button class="bg-red-600 text-white rounded px-4 py-2 hover:bg-red-700" @click="deleteComment(index)">Usuń</button>
            </div>
          </div>
  ...
  export default {
      ...
      methods: {
          ...
          async saveComment(index) {
              await dzien2_backend.edytuj_komentarz(this.report_index, index, this.editComment);
              this.editingComment = -1;
              await this.fetchReports();
          },
          async cancelEditComment() {
              this.editingComment = -1;
              this.editComment = "";
          },
